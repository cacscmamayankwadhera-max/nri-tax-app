        ...
        <div className="card-theme p-6 md:p-8 shadow-sm mb-6 animate-fade-in-up" style={{ animationDelay: '60ms' }}>
          ...
          <div className="relative stagger-children">
            {STAGES.map((s, i) => {
              ...
              return (
                <div key={s.key} className="flex gap-3 md:gap-4 mb-1 last:mb-0 animate-fade-in-up">
                  {/* Vertical line + circle */}
                  <div className="flex flex-col items-center" style={{ minWidth: 32 }}>
                    <div
                      className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-sm md:text-base flex-shrink-0 transition-all duration-500 ${
...
function ContactBar() {
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-theme-nav z-50 py-2 px-4" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
      <div className="max-w-3xl mx-auto flex flex-wrap items-center justify-center gap-3 md:gap-8 text-sm">
        <span className="text-theme-muted text-xs hidden md:inline">Questions?</span>
        <a
          href="https://wa.me/919667744073"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-1.5 text-green-400 hover:text-green-300 transition font-semibold text-xs min-h-[44px] px-3"
        >
        ...
        <a
          ...
          className="transition text-xs min-h-[44px] flex items-center px-3" style={{ color: 'var(--text-on-dark)' }}
        >
          tax@mkwadvisors.com
        </a>
        <a
