...
create policy "Users can create cases" on public.cases for insert with check (auth.uid() = user_id OR user_id IS NULL);
create policy "Users can update own cases" on public.cases for update using (auth.uid() = user_id);
-- Team members (admin/partner/senior) can see all cases
create policy "Team can view all cases" on public.cases for select using (
  exists (select 1 from public.profiles where id = auth.uid() and role in ('admin','partner','senior'))
);
create policy "Team can update all cases" on public.cases for update using (
  exists (select 1 from public.profiles where id = auth.uid() and role in ('admin','partner','senior'))
);
