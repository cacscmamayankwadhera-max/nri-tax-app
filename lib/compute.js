...
// TDS under Section 195: 30% of gain + surcharge + cess for NRI.
export function computeSTCGProperty(salePrice, purchaseCost, improvement = 0) {
  ...
  // STCG on property is added to normal income and taxed at slab rates
  // TDS under Section 195: 30% of gain + surcharge + cess for NRI
  const tds195 = Math.round(stcg * 0.30 * 1.04); // 30% + 4% cess
  return { stcg, tds195, taxRate: 'Slab rates (added to normal income)', note: 'Property held less than 24 months — Short-Term Capital Gain taxed at slab rates.' };
}
...
export function computeOldRegimeTax(totalIncome, deductions = {}) {
  const { sec80C = 0, sec80D = 0, sec80TTA = 0, sec80E = 0, sec80G = 0, other = 0 } = deductions;
  const totalDeductions = Math.min(sec80C, 150000) + Math.min(sec80D, 50000) + Math.min(sec80TTA, 10000) + sec80E + sec80G + other;
...
export function computeRegimeComparison(formData, fy) {
  ...
  const oldRegimeTax = oldRegime.total;
  // CG tax is same in both regimes (special rate), so add it to old regime
  const cgTax = ti.heads.find(h => h.head.includes('Capital Gains'))?.computation
    ? (ti.heads.find(h => h.head.includes('Capital Gains')).computation.better === 'A'
      ? ti.heads.find(h => h.head.includes('Capital Gains')).computation.optionA.total
      : ti.heads.find(h => h.head.includes('Capital Gains')).computation.optionB.total)
    : 0;
  const oldTotal = oldRegimeTax + cgTax;

  return {
    newRegime: { tax: newRegimeTax, label: 'New Regime (Section 115BAC)' },
    oldRegime: { tax: oldTotal, deductions: oldRegime.totalDeductions, label: 'Old Regime' },
...
// Stage 2: Capital gain from FMV at exercise to sale price — LTCG if held >12 months (listed).
export function computeESOPRSU(exercisePrice, fmvAtExercise, salePrice, holdingMonths) {
  ...
  const isLTCG = holdingMonths > 12; // Listed shares: 12 months for LTCG

  let cgTax, cgRate;
  if (isLTCG) {
    // Section 112A: 12.5% above ₹1.25L
