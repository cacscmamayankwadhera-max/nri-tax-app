...
export function computeHRAExemption(basicSalary, hraReceived, rentPaid, isMetro) {
  if (hraReceived <= 0) return { exempt: 0, taxable: 0 };
  if (rentPaid <= 0) return { exempt: 0, taxable: hraReceived };
  ...
  // Identify which rule produced the minimum (applied rule)
  let appliedRule;
  if (exempt === rule1) appliedRule = 'rule1';
  else if (exempt === rule2) appliedRule = 'rule2';
  else appliedRule = 'rule3';
...
export function computeTotalIncome(formData, fy) {
  ...
  if (f.salary) {
    ...
    if (f.basicSalary && f.hraReceived) {
      ...
      hraNotes = ` | HRA: received=₹${f.hraReceived}, exempt=₹${hraResult.exempt} (Rule ${hraResult.appliedRule}), taxable HRA=₹${hraResult.taxable}`;
    }
    heads.push({
      head: 'Salary',
