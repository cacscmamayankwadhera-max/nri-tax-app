...
// Fix 1: NRI applicability — post Finance (No.2) Act 2024, NRIs must use 12.5% flat rate only.
//        Dual option (20% indexed vs 12.5% flat) is only for resident individuals and HUFs.
// Fix 2: TDS under Section 195 is computed on LTCG at 12.5% + surcharge + cess, not on full sale price.
export function computeCapitalGains(salePrice, purchaseCost, acqFY, saleFY, improvement = 0) {
  ...
  // Option A: 20% with indexation (residents/HUFs only post July 2024)
  ...
  // Option B: 12.5% without indexation (mandatory for NRIs post July 2024)
  ...
  // `better` shows which is mathematically lower (useful for advisory memo / residents)
  ...
  // ── Fix 1: NRI applicability fields ──
  // Post Finance (No.2) Act 2024, NRIs MUST use Option B (12.5% flat).
  // The dual option is ONLY available for resident individuals and HUFs.
  const nriApplicable = "B"; // NRIs can only use Option B
  const nriNote = "Post Finance Act 2024, NRIs must use 12.5% flat rate. Dual option is for residents/HUFs only.";
  const nriTax = totalB; // What the NRI actually pays (always Option B)

  // ── Fix 2: TDS under Section 195 (NRI property sale) ──
  // TDS is computed on the LTCG (Option B, since that's what NRIs use) at 12.5% + surcharge + cess.
  // Surcharge rates based on LTCG amount (capped at 15% for LTCG):
  //   > 1Cr: 15% surcharge (capped for LTCG)
  //   > 50L: 10% surcharge
  //   <= 50L: no surcharge
  const ltcgForTDS = ltcgB; // Option B LTCG (no indexation) — what NRIs use
  let surchargeRate = 0;
  if (ltcgForTDS > 10000000) surchargeRate = 0.15;      // >1Cr, 15% surcharge (capped for LTCG)
  else if (ltcgForTDS > 5000000) surchargeRate = 0.10;   // >50L, 10% surcharge
  const tds195 = Math.round(ltcgForTDS * 0.125 * (1 + surchargeRate) * 1.04);
  const tdsRefund = tds195 - nriTax; // Refund based on what NRI actually pays (Option B)

  // Backward compat alias (old field name kept so existing UI references don't break)
  const tds194IA = tds195;

  // ── Section 54EC — correct tax-saved computation ──
  // Exemption: invest up to Rs 50L in NHAI/REC bonds within 6 months
  // Reduces LTCG by the invested amount; tax saved = difference
  // For NRIs, always compute using Option B (12.5%)
  const ec54Amount = Math.min(ltcgB, 5000000); // Use Option B LTCG for NRI context
  const remainingGain54EC = Math.max(0, ltcgB - ec54Amount);
  const taxWith54EC = Math.round(remainingGain54EC * 0.125 * 1.04);
  const sec54ecSaved = nriTax - taxWith54EC;
  ...
  return {
    ...
    better, savings,
    // Fix 1: NRI-specific fields
    nriApplicable,
    nriNote,
    nriTax,
    // TDS — Section 195 (NRI) — Fix 2: computed on LTCG, not sale price
    tds195,
    tdsRefund,
    ...
    // Net position (uses NRI-applicable tax = Option B)
    netTax: nriTax,
    netAfterTDS: nriTax - tds195,
...
export function computeSection54(ltcg, newHousePrice, section54Status) {
  ...
  // Fix 4: Tax on remaining at 12.5% + 4% cess (NRI rate post July 2024)
  const taxRate = 0.125;
  ...
  // Original tax (before exemption) at 12.5% + cess (NRI rate post July 2024)
...
export function computeAdvanceTax(totalTax, tds, fy) {
  ...
  return {
    ...
    schedule: [
      { date: '15 June ' + year1,      percent: 15,  amount: Math.round(balance * 0.15), cumulative: Math.round(balance * 0.15) },
      { date: '15 September ' + year1, percent: 30,  amount: Math.round(balance * 0.30), cumulative: Math.round(balance * 0.45) },
      { date: '15 December ' + year1,  percent: 30,  amount: Math.round(balance * 0.30), cumulative: Math.round(balance * 0.75) },
      { date: '15 March ' + year2,     percent: 25,  amount: Math.round(balance * 0.25), cumulative: balance },
    ],
...
export function computeTotalIncome(formData, fy) {
  ...
  if (f.propertySale && f.salePrice && f.purchaseCost) {
    ...
    // For NRIs, always use Option B LTCG (nriApplicable = "B")
    const nriLtcg = cgResult.optionB.ltcg;
    heads.push({
      head: 'Capital Gains (LTCG)',
      source: 'Property sale',
      amount: nriLtcg,
      computation: cgResult,
      note: `NRI: Option B (12.5% flat) applies. Option ${cgResult.better} is mathematically lower (saves ${formatINR(cgResult.savings)} for residents).`,
    });
    grossTotal += nriLtcg;
  }
  ...
  const cgTax = cgResult
    ? cgResult.optionB.tax   // NRI always uses Option B; use .tax (pre-cess) to avoid double cess
    : 0;
