...
export function buildCaseContext(formData, fy, moduleOutputs = {}) {
  ...
  if (formData.propertySale) {
    ...
    if (formData.propertySale && formData.saleDate && !formData.sec54ecInvestDate && (formData.section54||'').toLowerCase().includes('bond')) {
      ctx += `54EC PENDING: Investment date not recorded. Confirm ₹50L bonds within 6 months of ${formData.saleDate}.\n`;
    }
    ...
    if (formData.repatriationNeeded) {
      ctx += `Repatriation needed: Yes | Form 15CB: ${formData.form15CBStatus||'NOT TRACKED'} | Form 15CA: ${formData.form15CAStatus||'NOT TRACKED'}\n`;
      if (!formData.form15CBStatus || formData.form15CAStatus === 'Not yet filed') {
        ctx += `ACTION: Form 15CA/15CB not yet filed. Workflow: CA obtains Form 15CB → file Form 15CA online → submit to bank.\n`;
      }
    }
  ...
  if (formData.foreignTaxPaid && (!formData.form67Status || formData.form67Status === 'Not filed yet')) {
    ctx += `FORM 67 ALERT: FTC claimed but Form 67 not yet filed. FTC will be denied if Form 67 is not filed before ITR submission.\n`;
  }
  ...
  if (formData.section80EEB) ctx += `Section 80EEB (EV loan interest): ₹${formData.section80EEB} (old regime only)\n`;
  if (formData.indianAssets) ctx += `Indian assets: ${formData.indianAssets}\n`;
  if (formData.notes) ctx += `Notes: ${formData.notes}\n`;

  const residency = formData.residencyStatus || (parseInt(formData.stayDays) >= 182 ? 'Resident' : 'NR');
  const cflags = computeComplianceFlags({ ...formData, totalIncome: formData.totalIncomeEstimate || 0 }, residency);
  if (cflags.scheduleALRequired) ctx += `COMPLIANCE FLAG: Schedule AL mandatory (estimated income >₹50L).\n`;
  if (cflags.scheduleFARequired) ctx += `COMPLIANCE FLAG: Schedule FA mandatory (${residency} status).\n`;
  if (cflags.form15CARequired) ctx += `COMPLIANCE FLAG: Form 15CA/15CB required before remittance.\n`;
  if (cflags.cgasRequired) ctx += `COMPLIANCE FLAG: CGAS deposit required if Section 54 not invested by filing date.\n`;
