import { FY_CONFIG } from './compute';

const BASE = (fy) => {
  const cfg = FY_CONFIG[fy] || FY_CONFIG["2025-26"];
  return `You are part of the NRI Tax Suite for FY ${fy} (AY ${cfg.ay}). CII=${cfg.cii}. Default regime=New(115BAC). NRIs do NOT get ₹12L 87A rebate. Property pre-Jul-23-2024: dual computation (20% indexed vs 12.5% flat). Be precise, professional, advisory-led. Separate facts from assumptions. Flag missing data.`;
};
...
CRITICAL: For property acquired BEFORE July 23, 2024 — COMPUTE BOTH:
...
Also compute TDS position under Section 195 (NRI rate: ~20.8% of sale consideration). Flag potential TDS refund amount.
...
Revised DTAA provisions with UK, UAE, Singapore may reduce TDS rates.
Output:
...
- Asset disclosure ₹1Cr+ addressed?
...
ALWAYS include in every property-sale memo: dual CG computation comparison with exact savings figure.
...
ALWAYS mention: ₹12L 87A rebate does NOT apply to NRIs.
Output:
### Advisory Memo
**Client:** [name]
...
export function buildCaseContext(formData, fy, moduleOutputs = {}) {
  ...
  if (formData.rentalMonthly) ctx += `Rental: ₹${formData.rentalMonthly}/month\n`;
